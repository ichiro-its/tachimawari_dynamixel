...
int CM730::TxRxPacket(unsigned char *txpacket, unsigned char *rxpacket, int priority)
{
    ...
    if(priority > 0) m_Platform->MidPriorityRelease();
    if(priority > 1) m_Platform->LowPriorityRelease();
...
int CM730::GetRedByte(int color)
{
...
int CM730::GetGreenByte(int color)
{
...
int CM730::GetBlueByte(int color)
{
